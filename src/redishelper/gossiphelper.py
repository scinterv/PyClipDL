...
class GossipHelper(object):
    def __init__(self,host='localhost',port=6379,password=None,db=0,cluster=False):
        self.pool = ConnectionPool(host=host,port=port,password=None,decode_responses=True)
        self.r = Redis(connection_pool=self.pool)

        # Gossip Redis keys
        self._key_prefix="Gossip:"
        self.edge_id = self._key_prefix + "EdgeID"
        self.edge_set = self._key_prefix + "EdgeIDSet"
        self.rrd_edge_set = self._key_prefix + "RRDEdgeIDSet" # used for changing edge id
        self.time_cost = self._key_prefix + "TimeCost"

        self.ID = 1
        self.NAME = self._key_prefix + "edge:" + str(self.ID)
        self.htrain_sample ="{}:samples:edge{}".format(self._key_prefix,self.ID)
    ...
    def random_edgeid(self):
        try:
            lua = """
            local flag = redis.call('exists','{0}')
            if flag == 0 then
                redis.call('sunionstore','{0}','{1}')
            end
            return redis.call('spop','{0}')
            """.format(self.rrd_edge_set,self.edge_set)
            self.ID = self.r.eval(lua,0)
            self.NAME = self._key_prefix + "edge:" + str(self.ID)
            self.htrain_sample ="{}:samples:edge{}".format(self._key_prefix,self.ID)
        except Exception as e:
            print(repr(e))
            print("Get random edge id failed on {}!".format(self.NAME))
    ...
    def random_edge_id(self, can_be_self=False):
        ...
        try:
            ...
            while can_be_self is False and selid == self.ID:
                selid = int(self.r.srandmember(self.edge_set))
        except Exception as e:
            print(repr(e))
